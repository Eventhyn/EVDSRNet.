...
class Video_Provider_val_vimeo(Dataset):
    def __init__(self, base_path, txt_file, im_size=96, frames=5):
        super(Video_Provider_val_vimeo, self).__init__()
...
class Video_Provider_val_udm(Dataset):
    def __init__(self, base_path, txt_file, im_size=96, frames=5, sigma=10):
    ...
    def __getitem__(self, index):
        img_files = self._get_file_name(index)
        sigma = self._get_random_sigma(self.sigma)
...
class Video_Provider_val_davis(Dataset):
    def __init__(self, base_path, txt_file, im_size=96, frames=5,sigma=10):
        super(Video_Provider_val_davis, self).__init__()
        self.base_path = base_path
        self.txt_file = open(txt_file, 'r').readlines()
        self.im_size = im_size
        self.trans = transforms.ToTensor()
    ...
    def _get_file_name(self, index):
        ...
        """
        res = []
        path = os.listdir(self.base_path)
        path2 = self.base_path+"\\"+self.txt_file[index].strip()
    ...
    def __getitem__(self, index):
        ...
        print(len(img_files))
        sigma = self._get_random_sigma(self.sigma)
        HQ = torch.zeros(len(img_files), 3, 480, 832)
        LQ = torch.zeros(len(img_files), 3, 480, 832)
        for i, file in enumerate(img_files):
            img = Image.open(file)
            img = self.trans(img)[:, 0:480, 0:832]
            HQ[i,...] = img
        LR = torch.nn.functional.interpolate(HQ, scale_factor=0.25, mode='bicubic', antialias=True)
        noise = sigma * torch.randn_like(LR)
        LQ = LR + noise
